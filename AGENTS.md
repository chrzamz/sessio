...
自用工具：统一管理 Codex 和 Codex CLI 的对话历史。扫描本地 JSONL 文件，建立 SQLite 索引，提供全文搜索、按项目过滤、收藏标记等功能。解决对话散落多目录、改名后丢失、无法回溯的痛点。先自用，跑通后产品化发布。
...
- **为什么先做 Codex + Codex**：自己在用这两个，格式已摸清，后续通过 parser trait 扩展其他工具
...
### MVP 做什么
- [ ] 扫描 ~/.Codex 和 ~/.codex，解析 JSONL，写入 SQLite 索引
